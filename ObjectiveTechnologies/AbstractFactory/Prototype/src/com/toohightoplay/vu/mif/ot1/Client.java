...
import com.toohightoplay.vu.mif.ot1.factories.BarGuru;
...
public class Client {
	...
	public static void main(String[] args) throws YouShallNotPassException {
		...
		case REGULAR_USER:
			System.out
					.println("Hi client! You have joined as a regular browser");
			regularMenu();
			break;
		case HACKER:
		...
		case RANDOM_CHEATER:
			System.out
					.println("Hi random guy. I will let you come to our classical bar, but do not cheat again.");
			regularMenu();
			break;
		default:
			break;
	...
	public static void regularMenu() {
		...
		Bar bar = BarGuru.gotoClassicalBar();
	...
	public static void hackerMenu() throws YouShallNotPassException {
		MENU_STATE state = MENU_STATE.WAITING;
		Scanner scanner = new Scanner(System.in);
		Bar bar = BarGuru.gotoHackersBar();
		...
		while (state != MENU_STATE.GO_HOME) {
			...
			case WAITING:
				System.out
						.println("Bro wait a sec. I need to choose my weapon tonight");
				break;
			case DRINK:
				System.out.println();
				...
				System.out
						.println("1. With fireworks shooting when I found epic sword by the river, after I killed that little rabbit.");
				System.out.println("2. Get me some cold beer.");

				DRINKING_STRATEGY strategy = DRINKING_STRATEGY.values()[scanner
						.nextInt()];

				switch (strategy) {
				case NO_DRINK:
					throw new YouShallNotPassException(
							"Impossible decision. I need to terminate JAVA, OS and the universe by itself.");
				case SPECIAL_DRINK:
			...
			case GO_HOME:
				System.out
						.println("I am so drunk I can touch the sky. So long fella");
				break;
			default:
				System.out.println("Sorry Mister, we do not have that");
			}
